...
@polish "Harmonize the fully transformed prompt into one coherent final prompt without adding or removing requirements. Return the prompt itself and nothing else: no meta-commentary, no notes about what changed, and no restatement of these transformation instructions."
      ...
      @revise "If a Removal instruction block appears after the draft, apply it to the draft. If no Removal instruction block appears, preserve the draft." mode: editorial
        @revise "If an Additional section block appears after the draft, integrate it into the draft as a normal prompt section. If no Additional section block appears, preserve the draft." mode: editorial
          @revise "@{revision_instruction}" mode: editorial
            @normalize "Resolve cross-references and contradictions while preserving the prompt's intent. Where the source leaves ambiguous input unhandled, add a concrete rule for that case; where it states a MUST, give that requirement a concrete success criterion. Use the requirements inventory below as a preservation checklist; do not emit the inventory as a separate section in the final prompt." scope: semantic
...
@assert section: "Role and Identity"
@assert section: "Core Requirements"
@assert section: "Constraints and Prohibitions"
@assert section: "Output Format"
@assert section: "Edge Cases"
@assert not_contains: "restatement of these transformation instructions" severity: warning
