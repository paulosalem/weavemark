...
The first build is a complete vertical slice, not a prototype menu or static mockup. A player MUST be able to open one browser page, quickly learn the controls, start racing, complete or fail a round, see feedback about performance, and restart without reloading the page.
...
The game should deliver a compact arcade experience: fast restarts, readable hazards, satisfying craft control, and clear lap-based progression through a space course.
...
4. Clear controls taught on first load.
...
- mobile-specific virtual controls unless the implementation can add them cleanly without compromising keyboard play.

Optional enhancements MAY include sound effects, particle trails, screen shake, mobile touch controls, and local best-time persistence, but the game must remain playable without them.
...
Controls may differ, but the on-screen help must match the implementation exactly.
...
- Pressing the pause control SHOULD pause the game.
...
- `playing`: physics, controls, timer, collisions, and progress are active.
...
- Cap extreme velocities or forces to avoid tunneling and loss of control.
...
1. Player opens page.
2. Game displays title, objective, controls, and start prompt.
...
- ensure controls can be used from the keyboard,
- avoid flashing effects that could be uncomfortable.

Mobile support is optional, but the page should not break on narrower viewports. If mobile play is not supported, show keyboard-first controls and keep the layout readable.
...
- A player can open one browser page and understand the objective and controls.
