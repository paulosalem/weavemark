...
| [`prompt-refactoring-pipeline.weavemark.md`](promplets/catalog/standalone/prompt-refactoring-pipeline.weavemark.md) | Treats a messy prompt as a semantic artifact to refactor through a nested `@extract` → `@normalize` → `@revise` → `@expand` → `@polish` pipeline with `@assert` checks. |
