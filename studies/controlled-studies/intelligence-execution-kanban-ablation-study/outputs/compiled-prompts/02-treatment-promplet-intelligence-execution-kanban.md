...
- WeaveMark, prompt languages, and specification-driven prompting.
...
| --- | --- | --- | --- |
| Intake | New signals and manual captures | Inspect provenance and duplicates | Has triage status and score. |
...
### Source families
Official pages, feeds, newsletters, journals, repositories, social/community posts, forums, calendars, press releases, podcasts, videos, saved files, and manual imports.
...
| Triage inbox | Dedup clusters, strongest source, claim type, confidence, relevance rationale, accept/reject/watch-later controls. |
...
- Cards provide empty/loading/error/stale states, pointer and keyboard access, visible focus, sufficient contrast, semantic labels, and screen-reader text for icon-only controls.
...
- Date range picker controls dashboard widgets and tables with preset ranges and custom locale-aware range.

## 8. Automation, AI, alerts, and guardrails

### Automation rules
- Automation must show trigger, inputs, confidence, proposed/taken action, and undo/tuning controls.
...
- Noise controls: thresholds, mute rules, feedback, cooldowns, digest cadence, and per-topic preferences.
...
12. Fire or update an in-app alert/watch rule that explains why the item matters, what changed, confidence, next action, cooldown/digest behavior, and tuning controls.
