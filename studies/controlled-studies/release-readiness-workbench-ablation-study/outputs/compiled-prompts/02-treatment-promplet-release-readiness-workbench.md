...
- React Server Components by default; client components only for interactive boards, filters, forms, charts, browser-only controls, and live updates.
...
- Each row records command/manual flow, environment, expected proof, owner, status, failure meaning, release impact, and fix loop.
...
- Visible focus, semantic labels, sufficient contrast, and screen-reader text for icon-only controls.
...
- Validation matrix includes commands/manual flows, expected proof, owner, status, failure meaning, release impact, and attempt history.
