...
- mobile-perfect touch controls unless explicitly chosen during implementation.
...
Control MUST remain indirect. The player influences routes, timing, roles, and environment rather than manually steering every worker.
...
The first build MAY implement a smaller subset, but it must include enough role variety to demonstrate indirect crowd control.
...
Controls must be responsive and predictable. Illegal actions should fail clearly without corrupting simulation state.
...
5. **Validation/test layer**
   ...
   - Playwright browser smoke tests or manual MCP validation evidence.
